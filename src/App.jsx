...
const JSON_URL = null;
const STREAMS = [
  { name: "Star Sports 1 Tam", url: "https://dreamly-stream.pages.dev/SS1THD.html" },
  { name: "Star Sports 1 Eng", url: "https://dreamly-stream.pages.dev/SS1HDENG.html" },
  { name: "Star Sports 1 Hin", url: "https://dreamly-stream.pages.dev/SSHD1HIN.html" },
...
function getMatchStatus(match) {
    ...
    // diffDays: 0 if today, 1 if yesterday, negative if future
    const diffDays = Math.round((todayDate - matchDate) / 86400000);
    const totalMin = ist.getHours() * 60 + ist.getMinutes();

    if (diffDays === 0) {
      if (match.matchType === "day") {
        // Day match: 2 PM to 8 PM
        if (totalMin >= 14 * 60 && totalMin < 20 * 60) return "live";
        return totalMin < 14 * 60 ? "upcoming" : "completed";
      } else {
        // Evening match: 6 PM onwards (past midnight handled by diffDays === 1)
        if (totalMin >= 18 * 60) return "live";
        return "upcoming";
      }
    } else if (diffDays === 1) {
      // Evening match remains live until 2 AM next morning
      if (match.matchType === "evening" && totalMin < 2 * 60) return "live";
      return "completed";
    } else if (diffDays < 0) {
      return "upcoming";
    } else {
...
export default function App() {
  ...
  const filtered = withStatus.filter(m =>
    tab === "live" ? m._status === "live" : tab === "upcoming" ? m._status === "upcoming" : m._status !== "completed"
  );
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg-gradient)", fontFamily: "'Inter',system-ui,sans-serif", color: "var(--text-primary)", transition: "all 0.3s ease" }}>
        ...
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 14px", height: 58, display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 19, letterSpacing: 2, lineHeight: 1 }}>
              DREAMLY5<span style={{ color: "#f97316" }}>-IPL</span>
            </div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "14px 12px 50px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div>
                ...
                <div style={{ color: "rgba(255,255,255,0.9)", fontSize: 11, fontWeight: 900 }}>
                  <span style={{ color: "#f97316" }}>TAMIL HD</span> · <span style={{ color: "#f97316" }}>ENGLISH HD</span> · <span style={{ color: "#f97316" }}>Hindi HD</span> · <span style={{ color: "#f97316" }}>TELUGU HD</span> · {liveNow[0].matchType === "day" ? "2:00 PM – 8:00 PM" : "6:00 PM – 2:00 AM"} IST
                </div>
              </div>
        ...
        <div style={{ marginBottom: 12, borderRadius: 12, padding: "9px 13px", background: "var(--strip-bg)", border: "1px solid var(--strip-border)", display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span>☀️</span><div><div style={{ color: "var(--text-secondary)", fontWeight: 700, fontSize: 11 }}>Day</div><div style={{ color: "var(--text-muted)", fontSize: 9 }}>2:00 PM – 8:00 PM IST</div></div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 5, marginBottom: 12, background: "var(--tab-bg)", borderRadius: 13, padding: 4, border: "1px solid var(--strip-border)" }}>
          {[["all", `🏏 All (${withStatus.filter(m => m._status !== "completed").length})`], ["live", "🔴 Live"], ["upcoming", "🕐 Upcoming"]].map(([key, label]) => (
        ...
        <div style={{ marginTop: 24, textAlign: "center", color: "var(--text-muted)", fontSize: 10, lineHeight: 2 }}>
          <span style={{ color: "#f97316", fontWeight: 800 }}>Dreamly5-IPL</span> · Star Sports 1 Tamil HD · Star Sports 1 English HD · Star Sports 1 Hindi HD · Star Sports 1 Telugu HD<br />
